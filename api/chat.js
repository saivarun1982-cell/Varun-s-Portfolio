// api/chat.js — serverless function backing the "AR Assistant" chat widget.
//
// This calls Google's Gemini Interactions API (the current, recommended way
// to call Gemini models as of the @google/genai SDK — see
// https://ai.google.dev/gemini-api/docs/interactions). Nothing here should
// need editing except EDIT markers below.
//
// REQUIRED SETUP (see README.md "Chat setup" for full steps):
//   1. Create a Gemini API key at https://aistudio.google.com/apikey
//   2. In Vercel: Project Settings → Environment Variables → add
//      GEMINI_API_KEY with that value (all environments).
//   3. Redeploy. Until this is set, the widget shows a friendly
//      "not configured yet" message and the frontend automatically
//      falls back to canned answers from CHAT_FAQ in script.js.

const { GoogleGenAI } = require("@google/genai");

// EDIT: keep this in sync with your real skills/projects/links — the
// assistant is instructed to answer only from what's written here.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      reply: interaction.output_text || "I couldn't find an answer in the verified portfolio information."
    });
  } catch (error) {
